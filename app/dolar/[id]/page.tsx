...
export default function DolarPage({ params }: { params: { id: string } }) {
  const id = params.id
  ...
  const {
    ...
    queryKey: ['dolar'],
    queryFn: () =>
      fetch(`https://dolarapi.com/v1/dolares/${id}`).then(res => res.json()),
  })
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let result = Number(num) * dolar.compra
  ...
  return (
    <div className="min-h-screen gap-3 flex flex-col items-center pt-20 ">
      <p className="text-sm tracking-tight font-thin text-foreground">
        {' '}
        Ult. Actualización: {formatDate(new Date(dolar.fechaActualizacion))}
      </p>
      ...
      <Card className="p-5 shadow-xl space-y-2 bg-card text-accent-foreground">
        ...
        <CardDescription className="gap-5 flex ">
          <div className="space-x-2">
            <span>Compra:</span>
            <Badge variant="secondary">${dolar.compra}</Badge>
          </div>
          <div className="space-x-2">
            <span>Venta:</span>

            <Badge variant="secondary">${dolar.venta}</Badge>
          </div>
        </CardDescription>
      </Card>
